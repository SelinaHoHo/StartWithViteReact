...
function App() {
  ...
  return (
    <div style={{ display: "relative" }}>
      <div
        style={{
          backgroundImage: "linear-gradient(90deg, #A230ED, #5CDAFF)",
          height: "300px",
        }}
      ></div>
      <Space
        ...
        >
          ...
          >
            <span style={{ color: "rgba(0,0,0,.45)" }}>
              {myTasks.length} tasks left
            </span>
            <Button
              style={{ color: "rgba(0,0,0,.45)", padding: "0px" }}
              type="link"
              onClick={() => setMyTasks([])}
            >
              Clear all tasks
          ...
          {myTasks.map((item) => (
            <TaskCheck
              key={item.id}
              data={item}
              clickDelete={handleDeleteTask}
              clickUpdate={handleUpdateTask}
            />
          ))}
        </Card>
      </Space>
